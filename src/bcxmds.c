...
double bcxmds( const size_t n, double** delta, const size_t p, double** z, double** d, const size_t MAXITER, const double FCRIT, const double ZCRIT, size_t* lastiter, double* lastdif, const bool echo )
// Function bcxmds() performs linear multidimensional scaling.
{
  ...
  for ( iter = 1; iter <= MAXITER; iter++ ) {
    ...
    nnboxcox( symmetric, n, delta, d, 1.0 / ( 1.0 - fold ), NULL, gamma );
...
void Cbcxmds( int* rn, double* rdelta, int* rp, double* rz, double* rd, int* rmaxiter, double* rfdif, double* rzdif, double* rfvalue, int* recho )
// Function Cbcxmds() performs linear multidimensional scaling.
{
  // transfer to C
  size_t n = *rn;
  size_t p = *rp;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  double fvalue = bcxmds( n, delta, p, z, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, echo );
...
double bcxwgtmds( const size_t n, double** delta, double** w, const size_t p, double** z, double** d, const size_t MAXITER, const double FCRIT, const double ZCRIT, size_t* lastiter, double* lastdif, const bool echo )
// Function bcxwgtmds() performs weighted linear multidimensional scaling.
{
  const double EPS = DBL_EPSILON;                                          // 2.2204460492503131e-16
  const double TOL = sqrt( EPS );                                          // 1.4901161193847656e-08
  const double CRIT = sqrt( TOL );                                         // 0.00012207031250000000
  ...
  for ( iter = 1; iter <= MAXITER; iter++ ) {
    ...
    // compute transformation update
    nnboxcox( symmetric, n, delta, d, 1.0 / ( 1.0 - fold ), w, gamma );
...
void Cbcxwgtmds( int* rn, double* rdelta, double* rw, int* rp, double* rz, double* rd, int* rmaxiter, double* rfdif, double* rzdif, double* rfvalue, int* recho )
// Function Cbcxwgtmds() performs weighted linear multidimensional scaling.
{
  // transfer to C
  size_t n = *rn;
  size_t p = *rp;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  double fvalue = bcxwgtmds( n, delta, w, p, z, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, echo );
...
double fxdbcxmds( const size_t n, double** delta, const size_t p, double** z, int** fz, double** d, const size_t MAXITER, const double FCRIT, const double ZCRIT, size_t* lastiter, double* lastdif, const bool echo )
// Function fxdbcxmds() performs linear multidimensional scaling with fixed coordinates.
{
  const double EPS = DBL_EPSILON;                                          // 2.2204460492503131e-16
  const double TOL = sqrt( EPS );                                          // 1.4901161193847656e-08
  const double CRIT = sqrt( TOL );                                         // 0.00012207031250000000
...
void Cfxdbcxmds( int* rn, double* rdelta, int* rp, double* rz, int* rfz, double* rd, int* rmaxiter, double* rfdif, double* rzdif, double* rfvalue, int* recho )
// Function Cfxdbcxmds() performs linear multidimensional scaling with fixed coordinates.
{
  // transfer to C
  size_t n = *rn;
  size_t p = *rp;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  double fvalue = fxdbcxmds( n, delta, p, z, fz, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, echo );
...
double fxdbcxwgtmds( const size_t n, double** delta, double** w, const size_t p, double** z, int** fz, double** d, const size_t MAXITER, const double FCRIT, const double ZCRIT, size_t* lastiter, double* lastdif, const bool echo )
// Function fxdbcxwgtmds() performs weighted linear multidimensional scaling with fixed coordinates.
{
  const double EPS = DBL_EPSILON;                                          // 2.2204460492503131e-16
  const double TOL = sqrt( EPS );                                          // 1.4901161193847656e-08
  const double CRIT = sqrt( TOL );                                         // 0.00012207031250000000
...
void Cfxdbcxwgtmds( int* rn, double* rdelta, double* rw, int* rp, double* rz, int* rfz, double* rd, int* rmaxiter, double* rfdif, double* rzdif, double* rfvalue, int* recho )
// Function Cfxdbcxwgtmds() performs weighted linear multidimensional scaling with fixed coordinates.
{
  // transfer to C
  size_t n = *rn;
  size_t p = *rp;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  double fvalue = fxdbcxwgtmds( n, delta, w, p, z, fz, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, echo );
...
double varbcxmds( const size_t n, double** delta, const size_t p, const size_t h, double** q, double** b, double** d, const size_t MAXITER, const double FCRIT, const double ZCRIT, size_t* lastiter, double* lastdif, const bool echo )
// Function varbcxmds() performs restricted linear multidimensional scaling.
{
  // set constants
  const double EPS = DBL_EPSILON;                                          // 2.2204460492503131e-16
  const double TOL = sqrt( EPS );                                          // 1.4901161193847656e-08
...
void Cvarbcxmds( int* rn, double* rdelta, int* rp, int* rh, double* rq, double* rb, double* rd, int* rmaxiter, double* rfdif, double* rzdif, double* rfvalue, int* recho )
// Function Cvarbcxmds() performs restricted linear multidimensional scaling.
{
  // transfer to C
  size_t n = *rn;
  size_t p = *rp;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  double fvalue = varbcxmds( n, delta, p, h, q, b, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, echo );
...
double varbcxwgtmds( const size_t n, double** delta, double** w, const size_t p, const size_t h, double** q, double** b, double** d, const size_t MAXITER, const double FCRIT, const double ZCRIT, size_t* lastiter, double* lastdif, const bool echo )
// Function varbcxwgtmds() performs restricted weighted linear multidimensional scaling.
{
  const double EPS = DBL_EPSILON;                                          // 2.2204460492503131e-16
  const double TOL = sqrt( EPS );                                          // 1.4901161193847656e-08
  const double CRIT = sqrt( TOL );                                         // 0.00012207031250000000
...
void Cvarbcxwgtmds( int* rn, double* rdelta, double* rw, int* rp, int* rh, double* rq, double* rb, double* rd, int* rmaxiter, double* rfdif, double* rzdif, double* rfvalue, int* recho )
// Function Cvarbcxwgtmds() performs restricted weighted linear multidimensional scaling.
{
  // transfer to C
  size_t n = *rn;
  size_t p = *rp;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  double fvalue = varbcxwgtmds( n, delta, w, p, h, q, b, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, echo );
...
void bcxmds_unittest( long seed )
{
  char* dt = getdatetime();
  printstring( "\n", "==============================================================\n" );
  printstring( "started unit test bcxmds at ", dt );
  printstring( "\n", "==============================================================\n" );
  ...
  bcxmds( n, delta, p, z, d, MAXITER, FCRIT, ZCRIT, &lastiter, &lastdif, true );
  printscalar( "elapsed for bcxmds", getelapsedtime( tm ) );
  ...
  dt = getdatetime();
  printstring( "\n", "==============================================================\n" );
  printstring( "finished unit test bcxmds at ", dt );
  printstring( "\n", "==============================================================\n" );
} // unittest
